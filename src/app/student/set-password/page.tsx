...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    ...
    // The invite link's token exchange happens automatically as part of
    // createBrowserClient's session detection — just wait for the session
    // it establishes.
    ...
    <div className="flex min-h-screen items-center justify-center bg-cream-dim px-6 py-16">
      <div className="w-full max-w-sm">
        ...
        <div className="mt-8 rounded-2xl border border-navy/10 bg-white p-7 shadow-sm">
          {checkingSession ? (
            <p className="text-center text-sm text-navy/50">Verifying your invite link…</p>
          ) : !hasSession ? (
            <div className="flex items-start gap-2 rounded-xl border border-red/20 bg-red-soft px-4 py-3 text-sm text-red-dark">
              ...
              <p>
                This link has expired or was already used. Please contact the French Punjabi team for
                a new invite.
              </p>
            </div>
          ) : status === "done" ? (
