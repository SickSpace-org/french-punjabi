...
import { generateReadingPassage } from "@/lib/quiz/generateReadingPassage";
import { gradeSpeaking, type QuizMode } from "@/lib/quiz/gradeSpeaking";
...
export async function requestReadingPassage(): Promise<PromptResult> {
  const supabase = await createClient();
  const student = await getCurrentStudent(supabase);
  if (!student) return { ok: false, error: "Not authenticated." };

  try {
    const prompt = await generateReadingPassage();
    return { ok: true, prompt };
  } catch {
    return { ok: false, error: "Couldn't generate a passage. Please try again." };
  }
}
...
export async function submitSpeakingAttempt(
  ...
  let grade: QuizGrade;
  try {
    grade = await gradeSpeaking(mode, prompt, wavBase64);
  } catch {
